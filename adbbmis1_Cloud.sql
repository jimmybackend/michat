...
CREATE TABLE IF NOT EXISTS `UserAIAgentConfigs` (
  ...
  `scope_owner_key` int GENERATED ALWAYS AS ((case when (`scope` = _utf8mb4'global') then 0 else `user_id_` end)) STORED,
  ...
  CONSTRAINT `chk_uac_scope_owner` CHECK (((`scope` = _utf8mb4'global') and (`user_id_` is null)) or ((`scope` = _utf8mb4'user') and (`user_id_` is not null)))
