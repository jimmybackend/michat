...
try{
 ...
 $service=new SessionLifecycleService($db_connection);$row=$service->archive($userId,$sessionId);
 jexit(['ok'=>true]+$row);
...
catch(Throwable $e){error_log('chat2_session_archive.php: '.$e->getMessage());jexit(['ok'=>false,'error'=>'Error interno'],500);}
