...
if(isset($options['sleep'])){$sleep=filter_var($options['sleep'],FILTER_VALIDATE_INT);if($sleep===false||$sleep<1||$sleep>60){fwrite(STDERR,"Invalid --sleep\n");exit(2);}$config=new TaskWorkerConfig($config->workerId,$config->leaseSeconds,$sleep,$config->recoveryBatch,$config->recurrenceBatch,$config->recurrenceCatchUpLimit,$config->recurrenceRetryBatch,$config->recurrenceOrphanSeconds);}
...
$worker=new TaskWorker(new TaskClaimService($queue,$config),new TaskExecutionRunner(new TaskStepProgressionService($queue),new TaskLeaseService($queue,$config->leaseSeconds),$steps),new TaskRecoveryService($queue),$config,new TaskWaitService($queue),$recurrence);
