...
$runtime=$GLOBALS['task_chat_execution_service']??null;
if(!is_callable($runtime)){$runtime=static function():ChatExecutionResult{throw new RuntimeException('Server-side chat runtime is not configured');};}
$tools=new ToolRegistry();$registry=new TaskStepExecutorRegistry();
$registry->register('model',new ModelTaskStepExecutor(new ChatExecutionService($runtime)));$registry->register('tool',new ToolTaskStepExecutor($tools));$registry->register('validation',new ValidationTaskStepExecutor());$registry->register('finalize',new FinalizeTaskStepExecutor());$registry->register('approval',new ApprovalTaskStepExecutor());$registry->register('wait',new WaitTaskStepExecutor());$registry->register('plan',new PlanTaskStepExecutor());
$worker=new TaskWorker(new TaskClaimService($queue,$config),new TaskExecutionRunner($queue,new TaskLeaseService($queue,$config->leaseSeconds),new TaskStepExecutionService($registry)),new TaskRecoveryService($queue),$config);
