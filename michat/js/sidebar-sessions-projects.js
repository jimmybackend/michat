...
// Fase 4.1.1: chat.js es el UNICO propietario de sesiones/proyectos en Chat2.
// Este archivo se conserva como compatibilidad para otras vistas antiguas,
// pero en la pantalla principal no registra listeners, no hace fetch y no
// vuelve a crear sesiones. Evita el doble POST a chat2_session_create.php.
if (document.getElementById('pane-Chat2')) {
    window.__sidebarSessionsProjectsDelegatedToChatJs = true;
    console.log('ℹ️ Sidebar de sesiones/proyectos delegado a js/chat.js');
    return;
}
...
// ✅ CORREGIDO: Se eliminó el else con el prompt() redundante.
// El modal de gestión de proyectos SIEMPRE lo abre el listener
// registrado en chat.js (el. sbNewProject → openProjectManager).
// Este listener solo actúa como respaldo si openProjectManager
// estuviera disponible globalmente (no interfiere con el modal).
if (sbNewProject) {
...
// ✅ ELIMINADA la función createProject(name): ya no se usa porque
// ahora siempre se abre el modal de gestión de proyectos desde chat.js.
