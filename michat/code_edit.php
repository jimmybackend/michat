...
$activityRequestStartedAt = microtime(true);
$GLOBALS['activityTraceId'] = $activityTraceId;
$GLOBALS['activityUserId'] = $userId;
$GLOBALS['activitySessionId'] = 0;
$GLOBALS['activityTerminalEmitted'] = false;

// Liberar el lock de la sesión permite que chat_activity_poll.php consulte
// eventos mientras esta petición larga sigue ejecutando Bedrock/S3/lint.
if (session_status() === PHP_SESSION_ACTIVE) {
    session_write_close();
}

function codeActivityNormalize($value, int $depth = 0) {
    ...
    if (is_array($value)) {
        ...
        foreach ($value as $k => $v) {
            ...
            $out[$k] = codeActivityNormalize($v, $depth + 1);
        }
        return $out;
    }
    if (is_object($value)) return codeActivityNormalize((array)$value, $depth + 1);
    if (is_string($value)) {
...
function codeActivityEmit(
    ...
    $traceId = (string)($GLOBALS['activityTraceId'] ?? '');
    $sessionId = (int)($GLOBALS['activitySessionId'] ?? 0);
    $userId = (int)($GLOBALS['activityUserId'] ?? 0);
    ...
    try {
        $detailsJson = $details === null ? null : json_encode(codeActivityNormalize($details), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        $stmt = $db->prepare("INSERT INTO ChatActivityEvents (trace_id, session_id_, user_id_, phase, event_key, status, title, summary, details_json, model_id, duration_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if (!$stmt) return;
        $stmt->bind_param('siisssssssi', $traceId, $sessionId, $userId, $phase, $eventKey, $status, $title, $summary, $detailsJson, $modelId, $durationMs);
        $stmt->execute();
        $stmt->close();
    } catch (Throwable $e) {
        error_log('CODE_EDIT_ACTIVITY: ' . $e->getMessage());
    }
}

function codeActivityDuration(float $startedAt): int {
    return max(0, (int)round((microtime(true) - $startedAt) * 1000));
}

function jexit($arr, $code = 200) {
    if ($code >= 400 && empty($GLOBALS['activityTerminalEmitted'])) {
        $db = $GLOBALS['db_connection'] ?? null;
        if ($db instanceof mysqli) {
            $message = is_array($arr) ? (string)($arr['error'] ?? 'Error en code_edit.php') : 'Error en code_edit.php';
            codeActivityEmit($db, 'edit', 'trace_error', 'error', 'Edición interrumpida', $message, is_array($arr) ? $arr : null);
            $GLOBALS['activityTerminalEmitted'] = true;
        }
    }
...
$GLOBALS['activitySessionId'] = $sessionId;
...
$stmtSessionOwner = $db_connection->prepare("SELECT id_, project_id_ FROM ChatSessions WHERE id_=? AND user_id_=? LIMIT 1");
if (!$stmtSessionOwner) jexit(['ok'=>false,'error'=>'No se pudo validar la sesión.'], 500);
$stmtSessionOwner->bind_param('ii', $sessionId, $userId);
$stmtSessionOwner->execute();
$sessionOwnerRow = $stmtSessionOwner->get_result()->fetch_assoc();
$stmtSessionOwner->close();
if (!$sessionOwnerRow) {
    jexit(['ok'=>false,'error'=>'La sesión no existe o no pertenece al usuario.'], 403);
}
if (!empty($sessionOwnerRow['project_id_']) && (int)$sessionOwnerRow['project_id_'] !== $projectId) {
    jexit(['ok'=>false,'error'=>'La sesión seleccionada no pertenece al proyecto indicado.'], 409);
}

codeActivityEmit($db_connection, 'edit', 'trace_started', 'started', 'Edición directa iniciada', "{$action}: {$targetFilename}", [
    'action'=>$action,
...
codeActivityEmit($db_connection, 'edit', 'project_validated', 'completed', 'Proyecto validado', "Proyecto #{$projectId} pertenece al usuario.", [
    'project_id'=>$projectId,
    'root_prefix'=>$ownerRow['root_prefix'] ?? null,
]);
...
codeActivityEmit($db_connection, 'edit', 'source_resolved', 'completed', $isCreation ? 'Archivo nuevo preparado' : 'Fuente del proyecto localizada', $targetFilename, [
    'is_creation'=>$isCreation,
    'source_id'=>(int)($source['id_'] ?? 0),
    's3_key'=>$source['s3_key'] ?? null,
    'filename'=>$source['filename'] ?? $targetFilename,
]);
...
codeActivityEmit($db_connection, 'edit', 'source_loaded', 'completed', $isCreation ? 'Contenido inicial preparado' : 'Archivo leído desde S3', $isCreation ? 'Se generará un archivo nuevo.' : (strlen($currentContent) . ' bytes cargados.'), [
    's3_key'=>$source['s3_key'] ?? null,
...
if ($action === 'delete') {
    ...
    try {
        ...
        // 3. EmbeddingJobs no tiene FK a SourceChunks: limpiar antes de borrar la fuente.
        $stmtDelJobs = $db_connection->prepare("
            DELETE ej FROM EmbeddingJobs ej
            JOIN SourceChunks sc ON ej.target_type='source_chunk' AND ej.target_id=sc.id_
            WHERE sc.source_id_=?
        ");
        if ($stmtDelJobs) {
            $stmtDelJobs->bind_param('i', $source['id_']);
            $stmtDelJobs->execute();
            $stmtDelJobs->close();
        }

        // 4. Borrar la fuente (cascada elimina SourceChunks + ChunkEmbeddings, ver FKs)
        ...
        // 5. Borrar el historial de versiones de ESTE archivo (no tiene FK a ProjectSources)
...
$stmtInsert->execute();
$stmtInsert->close();
codeActivityEmit($db_connection, 'edit', 'version_draft_created', 'completed', 'Versión de trabajo creada', "v{$nextVersion} · FileVersions #{$newVersionId}", [
...
try {
    ...
    $classificationPromptForTrace = "Clasifica esta tarea de edición de código en una de estas categorías y responde SÓLO con la palabra clave:
- 'simple': cambios menores, corrección de errores, ajustes de una línea.
- 'medium': añadir funciones, modificar lógica moderada.
- 'complex': reescribir clases enteras, cambiar arquitectura, refactorización pesada.

Instrucción: " . $instruction;
    codeActivityEmit($db_connection, 'classify', 'classification_started', 'started', 'Clasificando complejidad', null, [
        'prompt'=>$classificationPromptForTrace,
        'model'=>'amazon.nova-micro-v1:0',
    ], 'amazon.nova-micro-v1:0');
    $category = classifyInstruction($instruction, $bedrock, $db_connection, $sessionId, $newVersionId);
    codeActivityEmit($db_connection, 'classify', 'classification_completed', 'completed', 'Complejidad clasificada', $category, [
        'prompt'=>$classificationPromptForTrace,
        'category'=>$category,
    ], 'amazon.nova-micro-v1:0', codeActivityDuration($classificationStartedAt));
    ...
    codeActivityEmit($db_connection, 'classify', 'classification_error', 'error', 'Clasificación en fallback', $e->getMessage(), ['fallback_ladder'=>$ladderBase], 'amazon.nova-micro-v1:0');
}

codeActivityEmit($db_connection, 'classify', 'model_ladder_ready', 'completed', 'Escalera de modelos preparada', (string)$category, [
    'complexity'=>$category,
    'ladder'=>$ladder,
]);
...
if (in_array($category, ['medium', 'complex'])) {
    ...
    $planPromptForTrace = "Eres un Arquitecto de Software. Analiza la instrucción y genera un PLAN DE EJECUCIÓN en JSON puro.
Identifica qué símbolos (clases, métodos, variables globales) se verán afectados para buscar referencias en otros archivos, y sugiere si hay tests que correr.
Formato JSON exacto (sin markdown):
{
  \"affected_symbols\": [\"NombreClase\", \"metodo\"],
...
Instrucción: {$instruction}
Archivo actual: {$targetFilename}";
    codeActivityEmit($db_connection, 'plan', 'plan_started', 'started', 'Generando plan de ejecución', null, [
        'prompt'=>$planPromptForTrace,
        'instruction'=>$instruction,
        'filename'=>$targetFilename,
    ], 'amazon.nova-micro-v1:0');
    $planData = generateExecutionPlan($instruction, $targetFilename, $bedrock, $db_connection, $sessionId, $newVersionId);
    codeActivityEmit($db_connection, 'plan', 'plan_completed', 'completed', 'Plan de ejecución generado', (string)($planData['plan_summary'] ?? 'Sin resumen de plan.'), [
        'result'=>$planData,
        'prompt'=>$planPromptForTrace,
    ], 'amazon.nova-micro-v1:0', codeActivityDuration($planStartedAt));
...
codeActivityEmit($db_connection, 'plan', 'impact_analysis_completed', 'completed', 'Impacto de la edición evaluado', ($impactAnalysis['is_multi_file'] ?? false) ? 'Se detectaron referencias en otros archivos.' : 'Edición contenida o sin referencias adicionales.', $impactAnalysis);
...
3. Ensure the code is syntactically perfect and ready to run.";
        ...
        foreach ($ladder as $tier) {
            for ($i = 0; $i < $tier['max_attempts']; $i++) {
                ...
                codeActivityEmit($db_connection, 'edit', 'generation_attempt_started', 'started', 'Generando archivo · intento ' . ($i + 1), $tier['model'], [
                    'system_prompt'=>$systemPromptCreator,
                ...
                codeActivityEmit($db_connection, 'edit', 'generation_attempt_completed', $lintResult['success'] ? 'completed' : 'error', $lintResult['success'] ? 'Archivo generado y validado' : 'Intento rechazado por lint', $lintResult['success'] ? 'Lint correcto.' : $lintResult['error'], [
                    'model'=>$tier['model'],
                    'attempt'=>$i + 1,
                    'tokens'=>['input'=>$inputTokens,'output'=>$outputTokens],
                    'model_output'=>$rawResponse,
                    'lint'=>$lintResult,
                ], $tier['model'], codeActivityDuration($attemptStartedAt));
        ...
        $contextEntityPromptForTrace = "Extrae ÚNICAMENTE los nombres de clases, funciones, métodos, interfaces o variables clave mencionados en esta instrucción de desarrollo. Devuelve un array JSON plano. Ejemplo: [\"UserRepository\", \"AuthService\", \"login\"].\nInstrucción: " . $instruction;
        codeActivityEmit($db_connection, 'rag', 'code_context_started', 'started', 'Buscando contexto relacionado', null, [
            'entity_extraction_prompt'=>$contextEntityPromptForTrace,
            'project_id'=>$projectId,
            'sql_strategy'=>'SourceChunks: name LIKE ? OR signature LIKE ?, LIMIT 5',
        ], 'amazon.nova-micro-v1:0');
        $relatedContext = fetchRelatedContext($db_connection, $projectId, $instruction, $bedrock, $sessionId, $newVersionId);
        codeActivityEmit($db_connection, 'rag', 'code_context_completed', 'completed', 'Contexto relacionado evaluado', $relatedContext !== '' ? 'Se recuperó contexto adicional del proyecto.' : 'No se encontró contexto adicional.', [
            'context'=>$relatedContext,
        ], 'amazon.nova-micro-v1:0', codeActivityDuration($contextStartedAt));
        
        $scoutStartedAt = microtime(true);
        codeActivityEmit($db_connection, 'scout', 'scout_started', 'started', 'Explorando archivo', 'Buscando el bloque exacto a modificar.', [
            'actual_input'=>"ARCHIVO (Total líneas: " . (substr_count($currentContent, "\n") + 1) . "):\n" . $currentContent . "\n\nINSTRUCCIÓN: " . $instruction,
            'model'=>'amazon.nova-micro-v1:0',
        ], 'amazon.nova-micro-v1:0');
        $scoutResult = scoutCodeBlock($currentContent, $instruction, $bedrock, $db_connection, $sessionId, $newVersionId);
        codeActivityEmit($db_connection, 'scout', 'scout_completed', 'completed', 'Bloque objetivo localizado', ($scoutResult['target_name'] ?? 'bloque') . ' · líneas ' . ($scoutResult['start_line'] ?? 1) . '-' . ($scoutResult['end_line'] ?? 1), $scoutResult, 'amazon.nova-micro-v1:0', codeActivityDuration($scoutStartedAt));
        $contextInfo = extractContext($currentContent, $scoutResult['start_line'], $scoutResult['end_line'], 15);
        codeActivityEmit($db_connection, 'edit', 'snippet_extracted', 'completed', 'Fragmento de edición extraído', 'Se añadió un buffer de contexto de ±15 líneas.', $contextInfo);
   ...
   - If you're unsure whether a change is requested, DO NOT make it. Preserve the original code.";
        ...
        foreach ($ladder as $tier) {
            for ($i = 0; $i < $tier['max_attempts']; $i++) {
                ...
                $attemptStartedAt = microtime(true);
                codeActivityEmit($db_connection, 'edit', 'edit_attempt_started', 'started', 'Editando bloque · intento ' . ($i + 1), $tier['model'], [
                    'system_prompt'=>$systemPromptEditor,
                    'user_prompt'=>$userPrompt,
                    'attempt'=>$i + 1,
                    'model'=>$tier['model'],
                    'scout'=>$scoutResult,
                    'inference_config'=>['maxTokens'=>2000,'temperature'=>0.1,'topP'=>0.9],
                ], $tier['model']);
                $res = $bedrock->converse([
                    'modelId' => $tier['model'],
                    'messages' => [['role' => 'user', 'content' => [['text' => $userPrompt]]]],
                ...
                codeActivityEmit($db_connection, 'edit', 'edit_attempt_completed', $lintResult['success'] ? 'completed' : 'error', $lintResult['success'] ? 'Edición reensamblada y validada' : 'Intento rechazado por lint', $lintResult['success'] ? 'Lint correcto.' : $lintResult['error'], [
                    'model'=>$tier['model'],
                    'attempt'=>$i + 1,
                    'tokens'=>['input'=>$inputTokens,'output'=>$outputTokens],
                    'model_output'=>$rawResponse,
                    'lint'=>$lintResult,
                    'scout'=>$scoutResult,
                    'lines_edited'=>($contextInfo['absolute_end'] - $contextInfo['absolute_start'] + 1),
                ], $tier['model'], codeActivityDuration($attemptStartedAt));
...
codeActivityEmit($db_connection, 'edit', 'code_ready', 'completed', 'Código listo para persistir', $targetFilename, [
    'is_creation'=>$isCreation,
    'attempt_log'=>$attemptLog,
...
codeActivityEmit($db_connection, 'edit', 'persistence_started', 'started', 'Guardando archivo y metadatos', $originalS3Key, [
    'target_filename'=>$targetFilename,
    ...
    if ($isCreation) {
        ...
        if ($existingSource) {
            ...
            $stmtUpdateSource = $db_connection->prepare("
                UPDATE ProjectSources
                SET filename = ?, mime_type = ?, size_bytes = ?, language = ?, status = 'pending', indexed_at = NULL
                WHERE id_ = ?
            ");
...
if ($files3Id > 0) {
    ...
    $db_connection->commit();
    $dbCommitted = true;
    codeActivityEmit($db_connection, 'edit', 'persistence_completed', 'completed', 'Archivo guardado', $originalS3Key, [
        's3_key'=>$originalS3Key,
        'source_id'=>(int)($source['id_'] ?? 0),
        'files3_id'=>$files3Id ?? null,
        'version'=>$nextVersion,
        'size_bytes'=>$fileSize ?? strlen($newContent),
    ], null, isset($persistStartedAt) ? codeActivityDuration($persistStartedAt) : null);
...
$summarySystemPromptForTrace = "Eres un revisor de código senior. Tu única tarea es describir, en un párrafo breve y profesional (máx. 80 palabras), qué se implementó en el archivo dado.
REGLAS OBLIGATORIAS:
1. NUNCA omitas nombres exactos de funciones, métodos, clases, variables o constantes relevantes que aparezcan en el código.
2. Menciona el nombre del archivo y si fue creado o editado.
3. No repitas el código completo, solo referencia los identificadores clave.
4. No uses markdown ni comillas, solo texto plano en español.
5. Sé técnicamente preciso: si detectas el lenguaje de programación, indícalo.";
$summaryActionForTrace = $isCreation ? 'CREACIÓN' : 'EDICIÓN';
$summaryUserPromptForTrace = "Acción: {$summaryActionForTrace}\nArchivo: {$targetFilename}\nInstrucción original: " . mb_substr($instruction, 0, 500) . "\n\nCódigo resultante:\n```\n" . mb_substr($newContent, 0, 6000) . "\n```\n\nDescribe qué se implementó, preservando los nombres exactos de funciones/variables/clases usados.";
codeActivityEmit($db_connection, 'review', 'summary_started', 'started', 'Generando resumen técnico', $targetFilename, [
    'system_prompt'=>$summarySystemPromptForTrace,
    'user_prompt'=>$summaryUserPromptForTrace,
...
codeActivityEmit($db_connection, 'review', 'summary_completed', 'completed', 'Resumen técnico generado', $diffSummary, $summaryResult, $summaryResult['model'] ?? null, codeActivityDuration($summaryStartedAt));
...
codeActivityEmit($db_connection, 'rag', 'indexing_started', 'started', 'Preparando índice del proyecto', $targetFilename, ['source_id'=>(int)($source['id_'] ?? 0)]);
try {
...
codeActivityEmit($db_connection, 'rag', 'indexing_completed', !empty($indexResult['ok']) ? 'completed' : 'error', 'Índice del proyecto preparado', !empty($indexResult['ok']) ? ((int)($indexResult['jobs'] ?? 0) . ' embedding(s) en cola.') : (string)($indexResult['error'] ?? 'Falló la indexación.'), $indexResult, $indexResult['model'] ?? null, codeActivityDuration($indexStartedAt));
...
codeActivityEmit($db_connection, 'edit', 'code_edit_completed', 'completed', 'Edición directa completada', "{$targetFilename} · v{$nextVersion}", [
    'filename'=>$targetFilename,
    'version'=>$nextVersion,
    'model_used'=>$attemptLog[count($attemptLog) - 1]['model'] ?? 'unknown',
    'complexity'=>$category ?? 'unknown',
    'index_result'=>$indexResult,
    'total_duration_ms'=>codeActivityDuration($activityRequestStartedAt),
]);
...
jexit([
    ...
    'index_chunks'    => (int)($indexResult['chunks'] ?? 0),
    'index_jobs'      => (int)($indexResult['jobs'] ?? 0),
    'embedding_model' => $indexResult['model'] ?? null,
    'index_error'     => !empty($indexResult['ok']) ? null : ($indexResult['error'] ?? null),
    // La preparación ya ocurrió dentro de ProjectIndexer; no repetir index_project_sources.php.
