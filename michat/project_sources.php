...
$user_id = (isset($_SESSION['user_id']) && is_numeric($_SESSION['user_id']))
    ? (int)$_SESSION['user_id']
    : 0;
if ($user_id <= 0) {
    jexit(['ok'=>false,'error'=>'No autenticado'], 401);
}
...
$effectiveEmbeddingModel = aiAgentActive('embedding_main', false)
...
if ($action === 'list') {
    ...
    $stmt->bind_param('si', $effectiveEmbeddingModel, $project_id);
    if (!$stmt->execute()) {
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        $chunksTotal = (int)($row['chunks_total'] ?? 0);
        $chunksReady = (int)($row['chunks_ready'] ?? 0);
        $effectiveStatus = (string)$row['status'];
        if ($effectiveEmbeddingModel !== '') {
            if ($chunksTotal > 0 && $chunksReady === $chunksTotal) {
                $effectiveStatus = 'indexed';
            } elseif ($effectiveStatus === 'indexed') {
        ...
        $sources[] = [
            ...
            'embedding_model' => $effectiveEmbeddingModel !== '' ? $effectiveEmbeddingModel : null,
...
if ($action === 'remove') {
    ...
    // EmbeddingJobs no tiene FK hacia SourceChunks: limpiar antes de borrar la fuente.
    ...
    // Eliminar fuente (cascada eliminará SourceChunks y ChunkEmbeddings)
...
if ($action === 'reindex') {
    ...
    $chk = $db_connection->prepare("SELECT ps.id_ FROM ProjectSources ps JOIN Projects p ON p.id_=ps.project_id_ WHERE ps.id_=? AND p.user_id_=? LIMIT 1");
    if (!$chk) jexit(['ok'=>false,'error'=>'Error preparando validación'], 500);
    $chk->bind_param('ii', $source_id, $user_id);
    $chk->execute();
    $owned = $chk->get_result()->fetch_assoc();
    $chk->close();
    if (!$owned) jexit(['ok'=>false,'error'=>'Fuente no encontrada o sin permisos'], 403);

    $db_connection->begin_transaction();
    try {
        $jobs = $db_connection->prepare("DELETE ej FROM EmbeddingJobs ej JOIN SourceChunks sc ON ej.target_type='source_chunk' AND ej.target_id=sc.id_ WHERE sc.source_id_=?");
        if ($jobs) { $jobs->bind_param('i',$source_id); $jobs->execute(); $jobs->close(); }

        $chunks = $db_connection->prepare("DELETE FROM SourceChunks WHERE source_id_=?");
        if (!$chunks) throw new RuntimeException($db_connection->error);
        $chunks->bind_param('i',$source_id);
        if (!$chunks->execute()) throw new RuntimeException($chunks->error);
        $chunks->close();

        $upd = $db_connection->prepare("UPDATE ProjectSources SET status='pending', indexed_at=NULL WHERE id_=?");
        if (!$upd) throw new RuntimeException($db_connection->error);
        ...
        jexit(['ok'=>false,'error'=>'No se pudo preparar reindexación: '.$e->getMessage()], 500);
    }

    jexit(['ok'=>true,'message'=>'Fuente invalidada y marcada para reindexar']);
}
