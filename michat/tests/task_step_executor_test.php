...
final class ExecutorTestApprovalStates implements TaskToolApprovalStateReaderInterface{
 public function __construct(private TaskToolApprovalState$approved){}public function read(int$id):TaskToolApprovalState{return in_array($id,[56,60],true)?$this->approved:TaskToolApprovalState::fromCheckpoint(null);}
}
final class ExecutorTestPauses implements TaskToolApprovalPauseInterface{public function pause(int$e,array$a,TaskToolApprovalProposal$p):array{throw new RuntimeException('unexpected_pause');}}
final class ExecutorTestConsumptions implements TaskToolApprovalConsumptionInterface{public function consume(int$e,string$k,array$a):array{return['consumed'=>true,'consumer_execution_id'=>$e,'fingerprint'=>str_repeat('a',64)];}}
...
$risk=new TaskToolRiskPolicy($tools);$proposalFactory=new TaskToolApprovalProposalFactory($risk,new TaskToolApprovalFingerprint());$dummyProposal=$proposalFactory->create('code_edit',['action'=>'write','target_filename'=>'Test.php'],['task_id'=>1,'step_id'=>1,'execution_id'=>1,'user_id'=>1,'project_id'=>null,'session_id'=>1]);$approvedCheckpoint=['tool_approval'=>['format_version'=>1,'identity'=>(new TaskToolApprovalIdentity(1))->toArray(),'proposal'=>$dummyProposal->toArray(),'decision'=>['status'=>'approved','fingerprint'=>$dummyProposal->fingerprint,'consumed'=>false]]];$approvalStates=new ExecutorTestApprovalStates(TaskToolApprovalState::fromCheckpoint(json_encode($approvedCheckpoint,JSON_THROW_ON_ERROR)));
$registry=new TaskStepExecutorRegistry();
$registry->register('model',new ModelTaskStepExecutor($chat));$registry->register('tool',new ToolTaskStepExecutor($tools,$artifactRepo,$risk,$approvalStates,$proposalFactory,new ExecutorTestPauses(),new ExecutorTestConsumptions()));$registry->register('validation',new ValidationTaskStepExecutor());$registry->register('finalize',new FinalizeTaskStepExecutor());$registry->register('approval',new ApprovalTaskStepExecutor());$registry->register('wait',new WaitTaskStepExecutor());$registry->register('plan',new PlanTaskStepExecutor());
...
$writeScope=['task_id'=>1,'step_id'=>1,'user_id'=>1,'session_id'=>2,'project_id'=>null];
...
$codeEdit=$service->execute($writeScope+['execution_id'=>60,'step_type'=>'tool','input'=>['tool_key'=>'code_edit']],$beat,$notCancelled);$ok($codeEdit->artifacts===$normalized&&array_slice($artifactRepo->records,-2)===[[60,80,'modified','project_source',7],[60,80,'generated','file_version',9]],'code_edit conserva sus dos artifacts normalizados');
...
$strResult=$service->execute($writeScope+['execution_id'=>56,'step_type'=>'tool','input'=>['tool_key'=>'str_replace']],$beat,$notCancelled);$ok($strResult->status==='completed'&&end($artifactRepo->records)===[56,79,'modified','project_source',7],'str_replace persiste un artifact con execution/toolCall del runtime');
$artifactRepo->fail=true;try{$service->execute($writeScope+['execution_id'=>56,'step_type'=>'tool','input'=>['tool_key'=>'str_replace']],$beat,$notCancelled);$ok(false,'error Repository impide completed');}catch(RuntimeException$e){$ok($e->getMessage()==='artifact_persist_failed','error Repository impide completed');}$artifactRepo->fail=false;
