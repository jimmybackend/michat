...
$ok(str_contains($repo,'private function persist(')&&str_contains($repo,'):int')&&str_contains($repo,'$this->db->insert_id')&&str_contains($repo,'return$id'),'record devuelve el insert_id real de MySQL');
$ok(str_contains($registry,'new ToolExecutionResult($result->summary,$result->artifacts,$result->data,$result->success,$result->status,$toolCallId)'),'registry propaga toolCallId sin alterar el resultado funcional');
