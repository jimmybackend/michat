...
try{
 ...
 $reconciliation=$root.'/michat/sql/fase12b_4_ai_scope_default_reconciliation.sql';
 $executor->executeFile($reconciliation);$default=$db->query("SELECT COLUMN_DEFAULT FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='UserAIAgentConfigs' AND COLUMN_NAME='scope'")->fetch_assoc()['COLUMN_DEFAULT'];$check($default===null,'reconciliation is a safe no-op when scope already has no default');
 $db->query("ALTER TABLE UserAIAgentConfigs ALTER COLUMN scope SET DEFAULT 'user'");$executor->executeFile($reconciliation);$default=$db->query("SELECT COLUMN_DEFAULT FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='UserAIAgentConfigs' AND COLUMN_NAME='scope'")->fetch_assoc()['COLUMN_DEFAULT'];$check($default===null,'12B.2C-compatible user default is removed');
 $db->query("ALTER TABLE UserAIAgentConfigs ALTER COLUMN scope SET DEFAULT 'global'");try{$executor->executeFile($reconciliation);$check(false,'unexpected scope default fails closed');}catch(Throwable$e){$check(str_contains($e->getMessage(),'unexpected scope default'),'unexpected scope default fails closed');}$db->query("ALTER TABLE UserAIAgentConfigs ALTER COLUMN scope DROP DEFAULT");
 $runner->baseline('current-dump');$history=$repository->fetchHistory();$check(count($history)===12&&count(array_filter($history,fn($r)=>$r['application_mode']==='clean_baseline'))===12,'current dump baseline atomically records 12 current checksums');
 $status=$runner->status();$check($status['global']==='APPLIED'&&count(array_filter($status['rows'],fn($r)=>$r['state']==='APPLIED'))===12,'baseline status is fully APPLIED');
 ...
 $db->query("DELETE FROM SchemaMigrations WHERE migration_id='fase12b_4_ai_scope_default_reconciliation'");$db->query("ALTER TABLE UserAIAgentConfigs ALTER COLUMN scope SET DEFAULT 'user'");$pending=$runner->status();$check($pending['global']==='PENDING'&&count(array_filter($pending['rows'],fn($r)=>$r['state']==='PENDING'))===1,'existing 12B.2C history leaves only reconciliation pending');$applied=$runner->apply();$default=$db->query("SELECT COLUMN_DEFAULT FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='UserAIAgentConfigs' AND COLUMN_NAME='scope'")->fetch_assoc()['COLUMN_DEFAULT'];$check($applied===['fase12b_4_ai_scope_default_reconciliation']&&$default===null,'apply runs only reconciliation and removes the historical default');
