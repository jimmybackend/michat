...
$ok(!str_contains($cli,'task_chat_execution'.'_service')&&str_contains($cli,'TaskStepExecutionServiceFactory'),'Worker compone el servicio compartido sin global');
$ok(str_contains(file_get_contents(__DIR__.'/../includes/Tasks/TaskStepExecutionServiceFactory.php'),'ToolRegistryFactory'),'Worker usa registry de producción compartido');
...
$context=file_get_contents(__DIR__.'/../includes/Chat/ChatContextPreparationService.php');$chatFactory=file_get_contents(__DIR__.'/../includes/Chat/ChatExecutionServiceFactory.php');
$ok(str_contains($cli,'TaskStepExecutionServiceFactory')&&str_contains($chatFactory,'ChatContextPreparationService'),'Worker y HTTP comparten preparación Memory/RAG');
$ok(!preg_match('/\$_(?:POST|GET|SESSION|COOKIE)\b/',$context),'preparación Worker no depende de superglobals');
$ok(str_contains($context,'ConversationScopeResolver')&&str_contains($context,'$scope->projectId()'),'ownership y proyecto se reconstruyen desde sesión persistida');
$projectGuard=strpos($context,"'has_project'=>\$scope->isProject()")<strpos($context,"'project_rag_context_block'");
$ok($projectGuard,'sin proyecto Router no solicita RAG de proyecto');
$response=file_get_contents(__DIR__.'/../includes/Chat/ChatResponsePersistenceService.php');
$ok(str_contains($repo,"'persist_final_response'=>")&&str_contains($response,'assignResultIfEmpty'),'Task async persiste una respuesta final compartida');
$ok(!preg_match('/\$_(?:POST|GET|SESSION|COOKIE)\b/',$response),'persistencia assistant del Worker no usa superglobals');
