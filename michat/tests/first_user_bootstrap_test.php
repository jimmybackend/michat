...
$check(str_contains($bootstrapExisting,"getenv('MICHAT_ACTOR_PASSWORD')")&&str_contains($bootstrapExisting,"getenv('MICHAT_BOOTSTRAP_CONFIRM')")&&!preg_match('/--password/', $bootstrapExisting),'existing-install bootstrap keeps password out of argv and requires explicit confirmation');
$check(str_contains($provision,'bootstrapExistingSuperadmin')&&str_contains($provision,"WHERE system_role='superadmin'")&&str_contains($provision,'superadmin_already_exists'),'existing-install bootstrap is one-time and only when no superadmin exists');
$check(str_contains($provision,'authenticateActiveUser($email,$password)')&&str_contains($provision,"SET system_role='superadmin' WHERE id=?"),'existing-install bootstrap promotes only the authenticated active target');
