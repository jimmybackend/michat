...
final class ChatResponsePersistenceService
{
    ...
    public function persist(int $taskId,int $userId,int $sessionId,string $content,string $modelId,string $traceId,array $usage=[],?string $stopReason=null,?int $latencyMs=null):int
    {
        ...
        try{
            ...
            $sql='INSERT INTO ChatMessages (id_,session_id_,user_id_,role,content_type,content,s3_key,mime_type,size_bytes,thumb_s3_key,duration_ms,model_id,stop_reason,prompt_tokens,completion_tokens,latency_ms,meta,is_primordial,phase,parent_msg_id) VALUES (?,?,?,?,?,?,NULL,NULL,NULL,NULL,NULL,?,?,?,?,?,?,?,?,?)';
            $stmt=$this->db->prepare($sql);if(!$stmt)throw new RuntimeException('database_error');
            $stmt->bind_param('iiisssssiiisisi',$messageId,$sessionId,$userId,$role,$type,$content,$modelId,$stopReason,$promptTokens,$completionTokens,$latencyMs,$meta,$primordial,$phase,$parent);
    ...
    {
        $result=$this->db->query('SELECT COALESCE(MAX(id_),0)+1 next_id FROM ChatMessages');
        if(!$result)throw new RuntimeException('database_error');return max(1,(int)$result->fetch_assoc()['next_id']);
    }
}
