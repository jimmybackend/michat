...
<form id="filters" class="filters" role="search"><label class="search">Buscar<input id="search" name="q" type="search" maxlength="200" placeholder="Título u objetivo"></label><label>Estado<select id="status" name="status"><option value="">Todos</option><option>pending</option><option>ready</option><option>running</option><option>waiting_user</option><option>waiting_dependency</option><option>completed</option><option>failed</option><option>cancelled</option></select></label><label>Prioridad<select id="priority" name="priority"><option value="">Todas</option><option value="low">Baja</option><option value="normal">Normal</option><option value="high">Alta</option><option value="urgent">Urgente</option></select></label><label>Proyecto<select id="project" name="project_id"><option value="">Todos</option></select></label><label>Sesión<select id="session" name="session_id"><option value="">Todas</option></select></label><button type="button" id="clear">Limpiar</button></form>
<div id="result-summary" class="result-summary" aria-live="polite"></div><div id="tasks" aria-live="polite"></div><section id="board" class="board" aria-label="Tablero operativo" aria-live="polite" hidden></section><nav class="pagination" aria-label="Paginación de Tasks"><button id="previous" type="button">Anterior</button><span id="page"></span><button id="next" type="button">Siguiente</button></nav></aside>
